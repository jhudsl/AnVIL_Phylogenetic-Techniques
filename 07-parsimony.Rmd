...
To test both NNI and SPR approaches for parsimony trees, we will use the `pratchet` command. This particular command has lots of possible parameters we can change.
  ...
  - perturbation: whether to use "ratchet" or "stochastic" (NNI) shuffling when shuffling the trees
...
tree.SPR <- pratchet(g.align, maxit = 10000, minit = 100, k = 10,
                           all = T, rearrangements = 'SPR', trace = 0, perturbation = 'ratchet')

tree.NNI <- pratchet(g.align, maxit = 10000, minit = 100, k = 10,
                           all = T, rearrangements = 'NNI', trace = 0, perturbation = 'stochastic')
...
We can see the bootstrap values for each branch by examining the tree objects directly. 

```{r,  warning=FALSE, message = FALSE}
#bootstrap value of each node 
SPR.root$node.label
```

This information can be helpful when a bootstrap value is difficult to read.
