...
calib <- makeChronosCalib(time.tree, node = 21, age.min = 2.6, age.max = 16.5)
...
Behold, you have a time tree. The branch lengths are now scaled to million of years, so you can estimate the age of MRCA between any two taxa on the tree. For example, our tree suggests the MRCA between mosquito grass and medusahead rye lived 10.98 MYA (the length of the branch between mosquito grass and the node that denotes the common ancestor with medusahead rye).
